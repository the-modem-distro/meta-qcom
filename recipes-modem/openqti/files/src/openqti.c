...
int init_port_mapper(struct qmi_device * qmidev) {
  ...
  ret = open_ipc_socket(qmidev, IPC_HEXAGON_NODE, IPC_HEXAGON_DPM_PORT, 0x2f, 0x1, IPC_ROUTER_DPM_ADDRTYPE); // open DPM service
  if (ret < 0) {
  ...
  do {
    sleep(1);
    logger(MSG_ERROR, "[%s] Waiting for the Dynamic port mapper to become ready... \n", __func__);
  } while (sendto(qmidev -> fd, & dpmreq, sizeof(dpmreq), MSG_DONTWAIT, (void * ) & qmidev -> socket, sizeof(qmidev -> socket)) < 0);
...
char *make_packet(int tid, char *command) {
  struct atcmd_reg_request *atcmd;
  char *buf;
  int k,i;
  ssize_t cmdsize;
  ssize_t bufsize;
  atcmd = (struct atcmd_reg_request *) calloc(1, sizeof(struct atcmd_reg_request));  
  //memset (atcmd,0xbb,sizeof(struct atcmd_reg_request));
 // atcmd->command = calloc(sizeof(char), 1); //WTF DOES IT NEED 43 padding zeros!
 //memset (atcmd->command,0x00,sizeof(atcmd->command));
  ...
  atcmd->packet_size =  strlen(command)+ 40 + sizeof(struct atcmd_reg_request); // atcmd + command + 36
  ...
  */
  ...
  bufsize = sizeof(struct atcmd_reg_request) + ((strlen(command) + 43) * sizeof(char));
  buf = calloc(1, 256);
  k = memcpy(buf, atcmd, sizeof(struct atcmd_reg_request));
  k = memcpy(buf+sizeof(struct atcmd_reg_request), command, (sizeof(char) * strlen(command)));
  return buf;
}
...
int init_atfwd(struct qmi_device *qmidev) {
  ...
  for (j = 0; j < (sizeof(at_commands) / sizeof(at_commands[0])); j++) {
    ...
    pktsize = sizeof(struct atcmd_reg_request) + ((strlen(at_commands[j].cmd)+47) * sizeof(char));
    ret = sendto(qmidev -> fd, atcmd, pktsize, MSG_DONTWAIT, (void * ) & qmidev -> socket, sizeof(qmidev -> socket));
    if (ret < 0) {
...
int handle_atfwd_response(struct qmi_device *qmidev, char *buf, unsigned int sz) {
  int j, sckret;
  int packet_size;
  ...
  char dummyok[] = { 0x00, 0x00, 0x00, 0x22, 0x00, 0x0b, 0x00, 0x01, 0x08, 0x00, 0x01, 0x00, 0x00, 0x00, 0x01, 0x03, 0x00, 0x00 };
...
int main(int argc, char ** argv) {
  ...
  for (i = 0; i < (sizeof(sysfs_value_pairs) / sizeof(sysfs_value_pairs[0])); i++) {
    if (write_to(sysfs_value_pairs[i].path, sysfs_value_pairs[i].value, O_RDWR) < 0) {
