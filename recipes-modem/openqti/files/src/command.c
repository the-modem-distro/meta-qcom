...
void set_custom_modem_name(uint8_t *command) {
  ...
  if (offset == NULL) {
    strcount = snprintf((char *)reply, MAX_MESSAGE_SIZE - strcount,
    ...
    if (strlen((char *)command) > ofs) {
      ...
      strcount = snprintf((char *)reply, MAX_MESSAGE_SIZE,
                          "My name is now %s\n", name);
      set_modem_name(name);
      get_names();
    }
...
uint8_t parse_command(uint8_t *command) {
  ...
  case 8:
    ...
    for (i = 0; i < (sizeof(bot_commands) / sizeof(bot_commands[0])); i++) {
      if (strlen(bot_commands[i].cmd) + (3 * sizeof(uint8_t)) +
              strlen(bot_commands[i].help) + strcount >
          MAX_MESSAGE_SIZE) {
        add_message_to_queue(reply, strcount);
        strcount = 0;
      }
  ...
  case 14:
    ...
    if (fp == NULL) {
      ...
      strcount += snprintf((char *)reply + strcount,
                           MAX_MESSAGE_SIZE - strcount, "Error opening file\n");
    } else {
