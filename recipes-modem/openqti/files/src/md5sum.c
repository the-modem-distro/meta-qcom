...
static int split_3(char *s, size_t s_len, unsigned char **u, int *binary,
                   char **w) {
  size_t i = 0;
...
/* An interface to md5_stream.  Operate on FILENAME (it may be "-") and
   ...
   to indicate success.  */
int md5_file(const char *filename, char *md5_result) {
  FILE *fp;
  fp = fopen(filename, "rb");
  if (fp == NULL) {
    logger(MSG_ERROR, "%s", filename);
    return FALSE;
  }

  if (md5_stream(fp, md5_result)) {
    logger(MSG_ERROR, "Error getting md5 for %s", filename);
    fclose(fp);
    return FALSE;
  }
